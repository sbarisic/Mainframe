...
public sealed class KernelIntegrationTests(Xunit.Abstractions.ITestOutputHelper output) : IDisposable
{
    ...
    public async Task IdentitySurvivesRestartAndQueriesUseAuthenticatedTls()
    {
        ...
        var identity = KernelStore.Initialize(path, "integration");
        using var store = KernelStore.Open(path);
        using var cert = store.LoadOperatorCertificate();
        using var ca = store.LoadCaCertificate();
        for (int restart = 0; restart < 2; restart++)
        {
            await using var server = new KernelServer(store, 0, output.WriteLine);
            server.Start();
            await using var client = await KernelClient.ConnectAsync("127.0.0.1", server.Port, cert, ca, TestContextToken());
            var description = await client.CallAsync("kernel.describe");
            ...
            var health = await client.CallAsync("kernel.health");
            Assert.Equal("ready", health.GetProperty("status").GetString());
            var capabilities = await client.CallAsync("kernel.capabilities");
            Assert.Equal(3, capabilities.GetProperty("capabilities").GetArrayLength());
        }
    }
    ...
    public async Task UntrustedServerAndClientCertificatesFail()
    {
        ...
        server.Start();
        using var validClient = first.LoadOperatorCertificate();
        using var wrongClient = other.LoadOperatorCertificate();
        using var validCa = first.LoadCaCertificate();
        using var wrongCa = other.LoadCaCertificate();
        await Assert.ThrowsAnyAsync<Exception>(async () =>
        {
            await using var ignored = await KernelClient.ConnectAsync("127.0.0.1", server.Port, validClient, wrongCa, TestContextToken());
        });
        await Assert.ThrowsAnyAsync<Exception>(async () =>
        {
            await using var ignored = await KernelClient.ConnectAsync("127.0.0.1", server.Port, wrongClient, validCa, TestContextToken());
        });
        await using var working = await KernelClient.ConnectAsync("127.0.0.1", server.Port, validClient, validCa, TestContextToken());
    ...
    public async Task UnsupportedMethodDoesNotBreakConnection()
    {
        ...
        using var ca = store.LoadCaCertificate();
        await using var client = await KernelClient.ConnectAsync("127.0.0.1", server.Port, cert, ca, TestContextToken());
        var error = await Assert.ThrowsAsync<KernelRpcException>(() => client.CallAsync("fs.open"));
    ...
    public async Task ReusedExchangeIdClosesConnection()
    {
        ...
        using var ssl = new SslStream(tcp.GetStream(), false, (_, certificate, _, errors) =>
            certificate is X509Certificate2 x509 && (errors & SslPolicyErrors.RemoteCertificateNameMismatch) == 0
            && CertificateTrust.Validate(x509, ca, CertificateTrust.ServerAuthentication));
        ...
        await FrameCodec.WriteAsync(ssl, new Frame(FrameType.Hello, 0, 0,
            ProtocolJson.Serialize(new HelloRequest([1], [], ["unary-rpc"], "terminal", "test", FrameCodec.MaxPayloadBytes))), deadline.Token);
        Assert.Equal(FrameType.Welcome, (await FrameCodec.ReadAsync(ssl, deadline.Token))!.Type);
        var request = new Frame(FrameType.Request, 1, 0,
            ProtocolJson.Serialize(new RpcRequest("kernel.health", 1, 5000, System.Text.Json.JsonSerializer.SerializeToElement(new { }))));
        ...
        try { Assert.Null(await FrameCodec.ReadAsync(ssl, deadline.Token)); }
        catch (IOException) { }
    }
    ...
    public async Task IdleConnectionSurvivesHeartbeatAndConcurrentClientCallsAreSerialized()
    {
        ...
        var results = await Task.WhenAll(Enumerable.Range(0, 12).Select(_ => client.CallAsync("kernel.health", cancellationToken: TestContextToken())));
    ...
    private string Init() { var path = NewState(); KernelStore.Initialize(path, "test"); return path; }
    ...
    public void Dispose()
    {
        ...
        if (!resolved.StartsWith(Path.GetFullPath(Path.GetTempPath()), StringComparison.OrdinalIgnoreCase)
            || !Path.GetFileName(resolved).StartsWith("Mainframe-Integration-", StringComparison.Ordinal))
