...
try {
    ...
    & cmd /d /c "`"$vcvars`" -vcvars_ver=14.44.35207 && nmake /nologo build_libs > openssl-build.log 2>&1"
    if ($LASTEXITCODE) { throw 'OpenSSL build failed; see openssl-build.log.' }
} finally { Pop-Location }
Push-Location $sqlcipher
try {
    & cmd /d /c "`"$vcvars`" -vcvars_ver=14.44.35207 && nmake /nologo /f Makefile.msc sqlite3.c > amalgamation.log 2>&1"
    ...
    & cmd /d /c "`"$vcvars`" -vcvars_ver=14.44.35207 && cl $arguments /I`"$openssl/include`" sqlite3.c `"$openssl/libcrypto.lib`" ws2_32.lib crypt32.lib advapi32.lib user32.lib /link /OUT:`"$runtime/sqlite3.dll`" > cipher-build.log 2>&1"
