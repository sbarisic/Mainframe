...
public static class CertificateTrust
{
    ...
    private static bool ValidateCore(X509Certificate2 certificate, X509Certificate2 ca, string expectedEku)
    {
        ...
        X509EnhancedKeyUsageExtension[] usages = certificate.Extensions.OfType<X509EnhancedKeyUsageExtension>().ToArray();
        ...
        return chain.Build(certificate) && chain.ChainElements.Count == 2 && string.Equals(Fingerprint(chain.ChainElements[^1].Certificate), Fingerprint(ca), StringComparison.Ordinal);
    }
}
