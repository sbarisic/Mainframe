...
public sealed partial class KernelStore
{
    ...
    public bool HasGrant(string principal, string permission)
    {
        using SqliteConnection database = Connect(directory);
        using SqliteCommand command = database.CreateCommand();
        command.CommandText = "SELECT COUNT(*) FROM grants WHERE principal=$p AND permission IN ('*',$g)";
        command.Parameters.AddWithValue("$p", principal);
        command.Parameters.AddWithValue("$g", permission);
    ...
    public static void ValidateManifest(ProgramManifest m)
    {
        ...
        if (m.Permissions.Any(s => s is not ("kernel.describe" or "kernel.health" or "kernel.capabilities")))
