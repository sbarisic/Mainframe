...
public abstract class KernelCommand : ICommand
{
    ...
    public string Usage => $"mframe {Name} [--state DIR] [--endpoint HOST:PORT] [--json]";
    ...
    public async Task<int> ExecuteAsync(CommandContext context, CancellationToken cancellationToken)
    {
        ...
        if (!File.Exists(caPath) || !File.Exists(operatorPath))
            throw new InvalidOperationException($"No operator credentials in '{context.StateDirectory}'. Run 'mframe cluster init' with this --state directory first.");
        using X509Certificate2 ca = X509CertificateLoader.LoadCertificateFromFile(caPath);
        ...
        using X509Certificate2 identity = X509CertificateLoader.LoadPkcs12FromFile(operatorPath, null, keyStorage);
        using var deadline = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        deadline.CancelAfter(TimeSpan.FromSeconds(20));
        await using KernelClient client = await KernelClient.ConnectAsync(endpoint.Host, endpoint.Port, identity, ca, deadline.Token);
